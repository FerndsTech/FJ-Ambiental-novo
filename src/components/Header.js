export function Header() {
  ...
  const fillHex = '#f8fafc'; // Hex do Slate-50
  ...
  return `
    <header id="main-header" class="fixed top-0 left-0 w-full z-[100] flex flex-col items-center pointer-events-none">
      ...
      <div class="relative flex items-center justify-between w-full md:w-auto pointer-events-auto">
        ...
        <div class="hidden md:block absolute top-0 right-full w-6 h-[80px]">
           <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none"><path d="M24 24C24 10.7452 13.2548 0 0 0H24V24Z" fill="${fillHex}"/></svg>
        </div>

        <div class="w-full md:w-auto bg-slate-50 h-[70px] md:h-[80px] px-6 md:px-10 rounded-b-[24px] md:rounded-b-[24px] flex items-center justify-between md:gap-10 shadow-sm md:shadow-none border-b border-slate-100 md:border-none">
          ...
          <button id="menu-toggle" aria-label="Abrir Menu" class="md:hidden relative z-[101] w-12 h-12 flex flex-col items-center justify-center gap-[6px] p-1 group outline-none">
        ...
        <div class="hidden md:block absolute top-0 left-full w-6 h-[80px]">
      ...
      <div id="mobile-menu" class="fixed inset-0 bg-slate-50 z-[90] flex flex-col justify-center items-center opacity-0 pointer-events-none">
          <nav class="flex flex-col items-center gap-6 text-3xl font-black text-slate-900 tracking-tight text-center">
            <a href="#home" class="menu-link overflow-hidden block w-full py-2 outline-none tap-highlight-transparent select-none"><span class="block translate-y-full hover:text-emerald-600 transition-colors">Home</span></a>
            <a href="#services" class="menu-link overflow-hidden block w-full py-2 outline-none tap-highlight-transparent select-none"><span class="block translate-y-full hover:text-emerald-600 transition-colors">Serviços</span></a>
            <a href="#portifolio" class="menu-link overflow-hidden block w-full py-2 outline-none tap-highlight-transparent select-none"><span class="block translate-y-full hover:text-emerald-600 transition-colors">Portfólio</span></a>
            <a href="#about" class="menu-link overflow-hidden block w-full py-2 outline-none tap-highlight-transparent select-none"><span class="block translate-y-full hover:text-emerald-600 transition-colors">Sobre</span></a>
          </nav>
